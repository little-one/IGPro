...
bool FolderMonitor::Run(LPCTSTR dir, bool bWatchSubtree, DWORD dwNotifyFilter, LPDEALFUNCTION dealfun, LPVOID lParam)
{
	m_hDir = CreateFile(dir,
		GENERIC_READ,
...
DWORD WINAPI FolderMonitor::Routine(LPVOID lParam)
{
	FolderMonitor& obj = *(FolderMonitor*)lParam;
...
void FolderMonitor::Close(DWORD dwMilliseconds)
{
	if (NULL != m_hThread)
	{
		m_bRequestStop = true;
...
FolderMonitor FolderMonitor::operator=(const FolderMonitor)
{
...
FolderMonitor::FolderMonitor() : m_hDir(INVALID_HANDLE_VALUE), m_hThread(NULL)
{
